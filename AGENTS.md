...
- Los cobros van en USD fijo: `PLAN_META` en `src/lib/billing.ts`. NO hay multi-moneda (se eliminó `CurrencyRate`, `User.currency` y los pickers de moneda). Precios siempre `Intl.NumberFormat("en-US", { style: "currency", currency: "USD" })`.
